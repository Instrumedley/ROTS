 ...
 please remove these rows.")
  ...
  if( is.null(a1) | is.null(a2) ){
    ...
    for(i in 1 : length(ssq)){
      ...
      .C("NeedForSpeed1", D, S, pD, pS, nrow(D), as.integer(N), length(N),
    ...
    .C("NeedForSpeed2", D, pD, nrow(D), as.integer(N), length(N),
       as.integer(B), overlaps, overlaps.P, DUP=FALSE)
